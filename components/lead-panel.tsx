...
export function LeadPanel({ lead, isOpen, onClose, onAddNote, onUpdateNote, onUpdateLead }: LeadPanelProps) {
  ...
  const handleStepClick = (stepId: number) => {
    ...
    // Add a note to mark this step as complete
    onAddNote(lead.id, {
      text: `Completed: ${clickedStep.action}`,
      type: clickedStep.type === "social" ? "note" : clickedStep.type,
    })
  }
  ...
  return (
    <Sheet open={isOpen} onOpenChange={() => onClose()}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto bg-black/40 backdrop-blur-xl border-system">
        <SheetHeader className="border-b border-system pb-4">
          <SheetTitle className="text-primary-hierarchy font-title">Lead Details</SheetTitle>
        </SheetHeader>

        {lead && (
          <div className="space-y-6 py-6">
            {/* Lead Info Card - Moved to top */}
            <Card className="bg-black/20 backdrop-blur-xl border-system rounded-3xl">
              <CardHeader>
                <CardTitle className="text-primary-hierarchy font-title text-lg">Lead Information</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Name</Label>
                    <div className="text-sm text-primary-hierarchy">{lead.name}</div>
                  </div>
                  <div>
                    <Label>Company</Label>
                    <div className="text-sm text-primary-hierarchy">{lead.company}</div>
                  </div>
                  <div>
                    <Label>Email</Label>
                    <div className="text-sm text-primary-hierarchy">{lead.email}</div>
                  </div>
                  <div>
                    <Label>Phone</Label>
                    <div className="text-sm text-primary-hierarchy">{lead.phone}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-black/20 backdrop-blur-xl border-system rounded-3xl">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-primary-hierarchy font-title">Property Address</CardTitle>
                  <div className="flex items-center gap-2">
                    {isEditingStatus ? (
